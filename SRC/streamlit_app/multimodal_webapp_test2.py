...
with tab2:
    ...
    # Initialize session state for all models
    ...
    if st.button(f'Train and Predict on {selected_modalities}'):
        ...
        # Train Intermediate Integration Model
        with st.spinner('Training Intermediate Integration model...'):
            intermediate_model = IntermediateIntegrationWrapper()
            intermediate_predictions = intermediate_model.wrapper(train_modalities, val_modalities, test_modalities)
            ...
            intermediate_test_performance = evaluate_model_on_preds(intermediate_predictions, test_modalities[selected_modalities[0]]['subtype'])
